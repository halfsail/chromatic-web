<script>
    ...
    import { gameData, increaseMove, increaseHints, completePuzzle } from '$lib/state/Store.svelte';
    import { getColors, shuffleColors, convertToP3 } from '$lib/utils/colorUtils.js';
            ...
            const draggable = Draggable.create(element, {
                ...
                onDrag: function () {
                    ...
                    draggableElements.forEach((dropTarget) => {
                        if (this.hitTest(dropTarget, "50%") && !dropTarget.classList.contains("dragover")) {
...
playFeedback('cellMove');
    ...
    async function getHint() {
        ...
        const indexOrder = [...Array(gameData.puzzle.col * gameData.puzzle.row).keys()];
...
>
    ...
    <section 
        class="board" 
        style="--colSize: {gameData.puzzle.col}; --rowSize: {gameData.puzzle.row};"
...
<style>
    ...
    .swatch {
        ...
        transition: box-shadow, scale, background, border-radius 200ms var(--shoot-ease);
        will-change: transform;
